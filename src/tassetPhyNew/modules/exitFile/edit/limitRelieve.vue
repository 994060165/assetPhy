  ...
  <el-dialog
    ...
    <el-row>
      <el-form ref="changeForm" :model="assetFlowInfo" :rules="rules" label-width="120px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="出门受限原因:" prop="outreason">
              <el-input
                type="textarea"
                :rows="2"
                placeholder="请输入内容"
                v-model="assetFlowInfo.outreason">
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      this.$refs['changeForm'].validate((valid) => {
        if (valid) {
          ...
          let params = {
            ...
            isoutlimit: true,
            outreason: this.assetFlowInfo.outreason
          }
          AssetChangeAPI.changeAssetForbidOut(params).then(data => {
            if (data.ID === '-1') {
              this.$message({
                type: 'error',
                message: `${data.msg}`
